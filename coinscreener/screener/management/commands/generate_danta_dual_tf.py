...
from coinscreener.screener.danta_dual_timeframe import (
    DANTA_PROFILES, SignalRejected, build_pullback_signal,
)
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--profile', choices=tuple(DANTA_PROFILES), default='v1')

    def _top_liquid_tickers(self, limit):
    ...
    def handle(self, *args, **options):
        profile = DANTA_PROFILES[options['profile']]
        now = timezone.now()
        today = timezone.localtime(now).date()
        try:
            candidates = self._top_liquid_tickers(profile.universe_limit)
        ...
        for row in candidates:
            ...
            try:
                ...
                signal = build_pullback_signal(hourly, five_minute, profile=profile)
            ...
            parameters = {
                ...
                'volume_expansion_multiplier': profile.volume_expansion_multiplier,
                'setup_window_minutes': profile.setup_window_candles * 5,
                'max_stop_loss_pct': 1.0,
                'maximum_kijun_distance_pct': profile.max_kijun_distance_pct,
                'minimum_risk_reward': profile.min_risk_reward,
                'first_take_profit_fraction': profile.first_take_profit_fraction,
            ...
            snapshot = recommendation_snapshot(
                profile.strategy_version, parameters,
            ...
            recommendation = DailyRecommendation.objects.create(
                ...
                reason=(
                    ...
                    f'{profile.label}: {signal["setup_candles_ago"] * 5}분 내 첫 눌림목 지지 확인. '
        ...
        if not created:
            ...
            return
        icon = '🚀' if profile.key == 'v2' else '🔥'
        lines = [f'{icon} <b>[{profile.label}] 1H/5M 눌림목 단타 신호</b>']
        for rec in created:
            lines.append(
