...
- `REWARDED_AD_UNIT_PATH` 보상형 광고 승인·설정 후에만 입력
- `SCAN_DAILY_FREE_LIMIT` 일일 무료 스캔 횟수, 기본 2
- `SCAN_DAILY_REWARD_LIMIT` 하루 광고 보상 상한, 기본 20
...
공개 검색은 브라우저 세션별 DB `ScanLease`를 획득한 요청 하나만 실행합니다. 여러 탭이나 중복 클릭으로 두 번째 검색이 들어오면 즉시 안내하고, 정상 종료·취소 시 실행권을 해제합니다. 프로세스 강제 종료로 실행권이 남아도 기본 30분 뒤 자동 만료됩니다.

KST 날짜별 `ScanUsage`는 무료 스캔과 보상 조회권을 계측합니다. 기본 무료 한도는 2회이며 Google Ad Manager의 `REWARDED_AD_UNIT_PATH`가 설정된 경우에만 실제 차단을 활성화합니다. 광고 단위가 없으면 계측만 수행합니다. `rewardedSlotGranted` 이벤트의 일회성 세션 nonce만 조회권 1회로 교환하며, nonce 재사용과 일일 과다 보상을 차단합니다.
